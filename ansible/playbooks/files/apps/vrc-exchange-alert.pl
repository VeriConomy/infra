...
sub bittrexTrade
{
    print "[DEBUG] Calling Bittrex API \n";
    # Get Order Book
    my $browser = LWP::UserAgent->new;
    my $resp = $browser->get('https://api.bittrex.com/v3/markets/' . $COIN . '-' . $TO . '/orderbook');
    ...
    foreach my $ask ( @{$data->{'ask'}} )
    {
        ...
        # checking if exist
        my $found = 0;
        foreach my $checkAsk ( @{$dataJson->{'bittrex-asks'}} )
        {
            my @checkAskArr = @{$checkAsk};
            if( $checkAskArr[0] ne $satPrice or $checkAskArr[1] ne $quantity ) 
            {
                next;
            }
            $found = 1;
        ...
        if( $found == 0 )
        {
            slackNotification($COIN . ' Alert - Someone submitted a sell order for ' . $quantity . ' ' . $COIN . ' at '. $satPrice . ' Satoshi on Bittrex');        
        }

        push(@foundAsks, [$satPrice, $quantity]);
    }

    my @foundBids = ();

    foreach my $bid ( @{$data->{'bid'}} )
    {
        my $price    = $ask->{'rate'};
        my $quantity = $ask->{'quantity'};
        ...
        # checking if exist
        my $found = 0;
        foreach my $checkAsk ( @{$dataJson->{'bittrex-bids'}} )
        {
            my @checkAskArr = @{$checkAsk};
            if( $checkAskArr[0] ne $satPrice or $checkAskArr[1] ne $quantity ) 
            {
        ...
        if( $found == 0 )
        {
            slackNotification($COIN . ' Alert - Someone submitted a buy order for ' . $quantity . ' ' . $COIN . ' at '. $satPrice . ' Satoshi on Bittrex');     
        }

        push(@foundBids, [$satPrice, $quantity]);
    }

    $dataJson->{'bittrex-asks'} = \@foundAsks;
    $dataJson->{'bittrex-bids'} = \@foundBids;
